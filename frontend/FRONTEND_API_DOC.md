...
// 请求体（FormData）
{
  task_id: "user123_MyYouTube_dQw4w9WgXcQ_a1b2c3d4",  // 必需
  file: File                                          // 字幕文件（.txt格式）
}

// 成功响应 200
{
  "message": "字幕文件已上传",
  "path": "cache/dQw4w9WgXcQ/raw/subtitle.txt"
}
...
// 使用场景：
// 当任务因字幕获取失败而失败时，用户可以手动上传字幕文件，然后重试任务
```
...
### Pipeline任务错误处理
当Pipeline任务执行失败时，可以通过状态接口获取详细错误信息：
...
const checkTaskStatus = async (taskId) => {
  ...
  // 检查是否有错误
  if (status.error) {
    console.error('任务执行错误:', status.error);
    
    // 特殊处理字幕获取失败
    if (status.error.includes('字幕获取失败') || 
        status.error.toLowerCase().includes('subtitle') ||
        status.error.toLowerCase().includes('transcript')) {
      
      // 提示用户上传字幕文件
      showSubtitleUploadDialog(taskId);
      return;
    }
...
// 手动上传字幕文件
const uploadSubtitle = async (taskId, file) => {
...
常见错误场景：
- **字幕获取失败**：YouTube视频没有字幕或字幕服务暂时不可用，用户可手动上传字幕文件
- **视频处理超时**：大型视频处理可能需要较长时间
- **资源不足**：服务器资源暂时不足，可稍后重试
